...
recursive(srcDir, [], (err, files) => {
  ...
  async.map(files, parseFiles, parseResult)

})

function getMinsToRead(text) {
  let totalWords = text.trim().split(/\s+/g).length;

	let wordsPerSecond = 150 / 60;

	let totalReadingTimeSeconds = totalWords / wordsPerSecond;

	return Math.round(totalReadingTimeSeconds / 60) || 1;
}
...
function parseFiles(file, loopCb) {
  ...
  let url = '/' + parsed.meta.slug + '/'

  result.description = parsed.meta.description
  result.url = url
  result.startingFilename = file
  result.markdown = cleaned
  result.minsToRead = getMinsToRead(cleaned)
  result.outputFile = url + 'index.html'
